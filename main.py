...
def get_test_data(path):
    x = np.load('%s/x.npy'%path).transpose((0, 2, 3, 1))[None].astype(np.float32)
    future = np.load('%s/future.npy'%path).transpose((0, 2, 3, 1))[None].astype(np.float32)
    y = np.load('%s/y.npy'%path)[None].astype(np.float32)*18.07
    return x, future, y
...
if __name__ == '__main__':
    ...
    x, future, y = get_test_data(args.data_path)
    print(x.shape, future.shape, y.shape)
    t_pred_long = 120
